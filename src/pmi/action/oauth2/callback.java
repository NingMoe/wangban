...
public class callback extends BaseAction {
	...
	public boolean handler(Map<String, Object> data) {
		...
		String client_secret = SecurityConfig.getString("CLIENT_SECRET");
		String auth_url = SecurityConfig.getString("AUTH_URL");
		...
		try {
			...
			String postResult = map.get("assertion");
			if(StringUtils.isEmpty(postResult)){
				postResult = this.getParameter("assertion");
			}
			String gotoUrl = map.get("goto");
			if(StringUtils.isEmpty(gotoUrl)){
				gotoUrl = this.getParameter("goto");
			}
			if (StringUtils.isEmpty(gotoUrl)) {
			...
			if(StringUtils.isEmpty(postResult)){
		    	...
		    	secureAttrs = client.buildUserInfo(postResult, client_secret, client_secret);
		    	
		    	this.setCookie(cookieName, secureAttrs.toString());
			}
			
			authCookie(secureAttrs.toString(), URLEncodeUtils.decodeURL(gotoUrl), url);
			return false;
		} catch (Exception e) {
			e.printStackTrace();
		}
