package action.icity.unionb2c.com.pay.consume;

import java.security.SecureRandom;
import java.util.HashMap;
import java.util.Map;

import action.icity.unionb2c.com.pay.DemoBase;
import action.icity.unionb2c.com.sdk.AcpService;
import action.icity.unionb2c.com.sdk.CertUtil;
...
public class Form_6_2_FrontConsume extends BaseAction {
	...
	public boolean handler(Map<String, Object> data) throws Exception {
		...
		CertUtil.initEncryptCert();// 初始化加密公钥证书
		CertUtil.initTrackKey();
		CertUtil.initValidateCertFromDir();// 初始化所有的验签证书
		
		Map<String, String> postData = this.getPostData();
		
		String merId = postData.get("merId");
		String txnAmt = postData.get("txnAmt");
		String reqReserved = postData.get("reqReserved");
		String sblsh = postData.get("sblsh");
		
		postData(merId,txnAmt,reqReserved,sblsh);
		
		return false;
	}
	
	public void postData(String merId,String txnAmt,String reqReserved,String sblsh){
		
		Map<String, String> requestData = new HashMap<String, String>();
		
		//4为随机数
		String ran = generateString(4);
		String orderId = DemoBase.getOrderId()+ran;
		...
		requestData.put("reqReserved", reqReserved);        		      //请求方保留域，如需使用请启用即可；透传字段（可以实现商户自定义参数的追踪）本交易的后台通知,对本交易的交易状态查询交易、对账文件中均会原样返回，商户可以按需上传，长度为1-1024个字节		
		...
		//生成报文html前将orderId更新到外网
		updateOrderId(sblsh,orderId);
