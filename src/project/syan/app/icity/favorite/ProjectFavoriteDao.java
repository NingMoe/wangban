...
public class ProjectFavoriteDao extends BaseJdbcDao {
	...
	public DataSet insert(ParameterSet pSet) {
		...
		int i = this.executeUpdate(sql, new Object[] { Tools.getUUID32(), (String) pSet.getParameter("SXBM"), userId,
				...
				(String) pSet.getParameter("REMARK"), (String) pSet.getParameter("STATUS"),
				(String) pSet.getParameter("SXMC"), (String) pSet.getParameter("SXID") });
		if (i == 0) {
	...
	public DataSet update(ParameterSet pSet) {
		...
		int i = this.executeUpdate(sql,
				new Object[] { (String) pSet.getParameter("SXBM"), (String) pSet.getParameter("UUID"),
						CommonUtils.getInstance().parseStringToTimeStamp((String) pSet.getParameter("FTIME"),
	...
	public DataSet addFaveriot(ParameterSet pSet) {
		DataSet ds = new DataSet();
		java.sql.Timestamp date = CommonUtils.getInstance().parseDateToTimeStamp(new java.util.Date(),
		...
		if (ret.getState() == 1 && ret.getTotal() > 0) {
			...
			int i = this.executeUpdate(sql, new Object[] {
					...
					(String) pSet.get("SXID"), (String) pSet.get("SXMC"), (String) pSet.get("SXBM"), pSet.get("USID") },
		...
		int i = this
				.executeUpdate(sql,
						new Object[] { Tools.getUUID32(), (String) pSet.get("SXID"), (String) pSet.get("SXBM"),
								(String) pSet.get("SXMC"), (String) pSet.get("USID"), date, "1" },
						this.getDataSourceName());
		if (i == 0) {
			ds.setState(StateType.FAILT);
			ds.setMessage("数据库操作失败！");
