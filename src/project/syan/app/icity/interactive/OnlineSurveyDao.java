package syan.app.icity.interactive;
...
public class OnlineSurveyDao extends BaseJdbcDao {
	...
	public DataSet saveAnswer(ParameterSet pSet, UserInfo userInfo) {
		...
		//更新答案表
		String sql = "insert into QUE_ANSWER(ID,SURVEY_ID,SURVEY_NAME, USER_ID, USER_NAME, IP, PHONE, CONTENT, SUBMIT_TIME) VALUES(?,?,?,?,?,?,?,?,?)";
		String userId = "";
				...
				for(int i=0; i<contentArr.size(); i++){
					...
					if(type == 0 || type == 1){
						...
						for(int j=0; j<options.size(); j++){
							...
							int checked = Integer.parseInt((String) option.get("checked"));
							if(checked == 0){
								int sum = Integer.parseInt( (String) joRes.getJSONArray("option").getJSONObject(j).get("sum") );
						...
						for(int j=0; j<options.size(); j++){
							...
							//if(checked == 0 && checkeds != 0){
								option.put("percent", String.valueOf(Integer.parseInt( options.getJSONObject(j).getString("sum") )/checkeds*100));
							//}
						}
					}else{
