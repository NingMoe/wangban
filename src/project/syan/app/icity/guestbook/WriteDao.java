...
public class WriteDao extends BaseJdbcDao {
	...
	public DataSet insertInfo(ParameterSet pSet) {
		String sql = "INSERT INTO GUESTBOOK "
				+ "(ID, TYPE, DEPART_NAME, USERNAME, PHONE, ADDRESS, TITLE, CONTENT,REGION_ID,WRITE_DATE,SXMC,USER_ID,"
	...
	public DataSet insert(ParameterSet pSet) {
		...
		/*String url1 = HttpUtil.formatUrl(
				SecurityConfig.getString("webSiteUrl") + "/web/c/getDictName?dicTypeCode=guestbook&dicCode=timeout");
		JSONObject json1 = JSONObject.fromObject(client1.getResult(url1, ""));
		JSONArray ja = (JSONArray)json1.get("rows");
		String timeout = (String) ((JSONObject)ja.get(0)).get("NAME");*/
		
		// 获取超期2天的日期
		HttpClientUtil client = new HttpClientUtil();
		String url2 = HttpUtil.formatUrl(
				SecurityConfig.getString("webSiteUrl") + "/web/c/getWorkDate?startTime=" + temp_str + "&timeout="+timeout);
		...
		try {
			...
			if(StringUtils.isNotEmpty(deptname)){
				...
				String url1 = HttpUtil.formatUrl(
						SecurityConfig.getString("webSiteUrl") + "/web/c/getWorkDate?startTime=" + temp_str + "&timeout=1");
				JSONObject json1 = JSONObject.fromObject(client1.getResult(url1, ""));
				j4 = CommonUtils_api.getInstance().parseStringToTimestamp(json1.getString("data"),
						CommonUtils_api.YYYY_MM_DD);
			}
		} catch (Exception e1) {
			e1.printStackTrace();
		...
		String sql = "INSERT INTO GUESTBOOK "
				...
				+ "STATUS,REGION_ID,SXID,SXBM,SXMC,URL,DESIGN,PLEASE,ZJHM,ACCEPT,ACCEPTTIMEOUT,DISPENSETIMEOUT,ACCEPTTIMEWARN)"
				+ "VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		DataSet ds = new DataSet();
		String id = Tools.getNewID("syan");// Tools.getUUID32();//Tools.getNewID("mutual");
		int i = 0;
		try {
			i = this.executeUpdate(sql,
					new Object[] { id, // id生成器生成
							...
							(String) pSet.getParameter("PLEASE"), (String) pSet.getParameter("ZJHM"), "0", j2,j3,j4});
		} catch (Exception e) {
			e.printStackTrace();
		}
		if (i == 0) {
	...
	public DataSet getGuestBookList(ParameterSet pSet) {
		...
		try {
			...
			String sql = "select t.id,t.depart_name,t.title,t.status,t.write_date,open" 
					+ " from guestbook t"
					+ " where t.type = '" + type + "'"
