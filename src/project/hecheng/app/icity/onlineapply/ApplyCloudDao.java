package hecheng.app.icity.onlineapply;
...
public class ApplyCloudDao extends BaseJdbcDao{
	...
	public JSONObject submitData(JSONObject data) throws Exception {	
		System.out.println("一体机的data："+data.toString());
		...
		try{
			if("sp".equals(state)||"ql".equals(state)||"edit".equals(state)){
				
				//预留，准备增加查询密码；
				jo = insertComCloud(data);
				
				System.out.println("看看二次调用返回的值："+jo.toString());
				String status = "00";
				if("200".equals(jo.getString("state"))){
					status = "11";
					sblsh = jo.getString("receiveNum");
					//cxmm = jo.getString("password");//查询密码//等云受理返回查询密码后再补充
					data.put("retstate", "1");							
				}else{//提交失败 设为暂存
