...
/**
 * 昆明、盘龙项目使用办件推送插件
 * 由于昆明项目 和盘龙 项目 部署一套系统，共用数据库。
 * 昆明办件推送到审批、盘龙项目推送到云受理，为避免产品推送代码混乱，因此单独建立昆明推送方法
 * @author yanhao
 *
 */
public class ApplyKMDataChangeDao extends BaseJdbcDao {

	private static Log log_ = LogFactory.getLog(ApplyKMDataChangeDao.class);

	private ApplyKMDataChangeDao() {
		this.setDataSourceName("icityDataSource");
	}

	public static ApplyKMDataChangeDao getInstance() {
		return (ApplyKMDataChangeDao) DaoFactory.getDao(ApplyKMDataChangeDao.class.getName());
	}
	...
	public void handleData() {
		...
		String reginCode="";//定义区划变量 区分盘龙办件
		try {
			conn = DbHelper.getConnection(this.getDataSourceName());
			conn.setAutoCommit(false);
			// 一次处理50条数据
			//增加 i.XZQHDM 行政区划代码
			sql = "select i.sblsh,i.SBXMMC,i.XZQHDM,s.basecontent from business_index i inner join sub_for_ex_app_information s on i.sblsh=s.sblsh "
					+ " where i.signstate='1'"
			...
			for (int i = 0; i < ds.getTotal(); i++) {
				...
				try {
					...
					reginCode = ds.getRecord(i).getString("XZQHDM");
					...
					if (j > 0) {
						...
						if (!"B".equals(power_source)) {
							JSONObject ret;
							//盘龙区及其下属街道-社区 办件 走云受理
							if(reginCode.startsWith("530103")){
								log_.error("推送到云受理");
								error_type ="线申报数据推向云受理（盘龙）";
								ret = insertComCloud(data);//推送到云受理
								log_.error("推送到云受理："+ret);
							}else{//盘龙区外其他地区办件走 审批系统
								error_type ="在线申报数据推向审批";
								log_.error("推送到审批");
								ret = insertComApproval(data);//推送到审批
								log_.error("推送到审批："+ret);
							}
				...
				try{
					...
					log_.error("异常信息"+e);
					error_type ="在线申报数据推向汇总库";
					content = "推送前，业务：[" + sblsh + "]，写入前置库异常。message:"
							+ e.toString();
					Plugin_Util.getInstance().insertErrorLog(
							error_type, error_name, error_sign, content);
						updateBusiness(sblsh, "3");
				}
			}
	...
	//推送到云受理
	public JSONObject insertComCloud(JSONObject data) {
		try {
	...
	//推送到审批
	public JSONObject insertComApproval(JSONObject data) {
		try {
			String url  = HttpUtil.formatUrl(SecurityConfig
	...
	public void insertUnify(JSONObject data, String version, String power_source)
		...
		try {
			...
			connQZ.rollback();
			log_.error("申报数据推向汇总库异常",e);
			log_.error("申报数据推向汇总库异常"+e.getMessage());
			e.printStackTrace();
			throw e;
		} finally {
