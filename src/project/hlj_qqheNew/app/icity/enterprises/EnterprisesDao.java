  ...
  public DataSet saveEnterprise_business_course(ParameterSet pSet){
    ...
    try{
      ...
      if("zs".equals(SecurityConfig.getString("AppId"))){
    	  ...
    	  _ds = getApplicationInfo_qqhe(pSet);
      }
      if(_ds.getState()==1){
...
*/
  ...
  public DataSet saveEnterprise_lhtk_course(ParameterSet pSet){
	...
	public DataSet getScopeTree(ParameterSet pSet){
		DataSet ds = new DataSet();
	    String url = HttpUtil.formatUrl(SecurityConfig.getString("EAParallelUrl")+"/getScopeTree");
	    Map<String, String> data=new HashMap<String, String>();
	    JSONObject receive = new JSONObject();
	    try{
	      receive =  JSONObject.fromObject(RestUtil.postData(url, data));
	...
	public DataSet getApplicationInfo_qqhe(ParameterSet pSet){
		DataSet ds = new DataSet();
		String flowId = ((JSONObject)getFlow(pSet).getJAData().get(0)).get("flowId").toString();
	    String url = HttpUtil.formatUrl(SecurityConfig.getString("EAParallelUrl")+"/getApplicationInfo");
	    Map<String, String> data=new HashMap<String, String>();
	...
	public DataSet getItemByScope(ParameterSet pSet){
		DataSet ds = new DataSet();
	    String url = HttpUtil.formatUrl(SecurityConfig.getString("EAParallelUrl")+"/getItemByScope");
	    Map<String, String> data=new HashMap<String, String>();
	    JSONObject receive = new JSONObject();
	  ...
	  public DataSet submitApplicationInfo_qqhe(ParameterSet pSet){
	    DataSet ds = new DataSet();
	    String selectItem = "";
	    JSONArray dsItem = getItemByScope(pSet).getJAData();
	    for(int i=0;i<dsItem.size();i++){
	    	selectItem = selectItem+((JSONObject)dsItem.get(i)).get("itemId").toString()+",";
	    }
	    String url = HttpUtil.formatUrl(SecurityConfig.getString("EAParallelUrl")+"/submitApplicationInfo");
	    Map<String, String> data=new HashMap<String, String>();
	    data.put("selectItem", selectItem.substring(0, selectItem.length()-1));
	    data.put("scopeCode", (String)pSet.get("scopeCode"));
	    data.put("dataId", (String)pSet.get("dataId"));
	    data.put("flowId", (String)pSet.get("flowId"));
	    data.put("material", pSet.get("material").toString());
	    JSONObject receive = new JSONObject();
	    try{
	      ...
	      if(receive.get("state").equals("200")){
	          //这里的错误通常是dcid没有获取到。
	      	String dcId = receive.get("bizId").toString();
	        pSet.setParameter("dcId", dcId);
	        DataSet _ds = saveEnterprise_business_index(pSet);
	        if(_ds.getState()==1){
	          ds.setState(StateType.SUCCESS);
