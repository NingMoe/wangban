...
public class SatisfactionEvaluationDao extends BaseJdbcDao {
	...
	public DataSet insertNewEvaluation1(ParameterSet pSet) {
		...
		System.out.println(pSet.getParameter("SBLSH") );
		ds = this.executeDataset(sql, new Object[] { pSet.getParameter("SBLSH") });
		if (ds.getTotal() > 0) {
			// 评价过了
		...
		sql = "insert into STAR_LEVEL_EVALUATION(ID,BSR_NAME,SERIAL_NUMBER,STAR_LEVEL,EVALUATE_CONTENT,"
				...
				+ "SERVICE_ORG_NAME,REGION_CODE,APPLY_NAME,TIME_STAR_LEVEL,QUALITY_STAR_LEVEL,MAJOR_STAR_LEVEL,CONVENIENCE_STAR_LEVEL,CRYPTONYM) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		try {
			int i;
			i = this.executeUpdate(sql,
					new Object[] { strId, strBSRName, strSerialNumber, strStarLevel, strEvaluateContent, remoteAddr,
							...
							ds.getRecord(0).getString("SJDW"), ds.getRecord(0).getString("XZQHDM"),
							ds.getRecord(0).getString("SBXMMC"), strTimeStarLevel, strQuueryStarLevel,
		...
		try {
			...
			i = this.executeUpdate(sql,
					new Object[] { strId, strBSRName, strSerialNumber, strStarLevel, strEvaluateContent, remoteAddr,
							...
							strQuueryStarLevel, strMajorStarLevel });
	...
	public DataSet queryStarLevel(ParameterSet pSet) {
		DataSet ds;
		String sql = "select SERIAL_NUMBER,STAR_LEVEL,EVALUATE_CONTENT from STAR_LEVEL_EVALUATION where SERIAL_NUMBER = ?";
		ds = this.executeDataset(sql, new Object[] { pSet.getParameter("sblsh") });
		return ds;
	}
	...
	public DataSet getSbsxlist(ParameterSet pSet) {
		...
		String sql = "select * from BUSINESS_INDEX where STATE in ('02','13','97','98','99') and UCID = ? and XZQHDM ='"+pSet.remove("XZQHDM")+"' and not exists  (select 1 from star_level_evaluation where serial_number = sblsh)";
		try {
	...
	public DataSet insertEvaluation(ParameterSet pSet) {
		...
		sql = "insert into STAR_LEVEL_EVALUATION(ID,BSR_NAME,SERIAL_NUMBER,STAR_LEVEL,EVALUATE_CONTENT,"
				+ "CLIENT_IP,CREATOR_ID,CREATOR_DATE,SERVICE_CODE,SERVICE_ID,SERVICE_NAME,SERVICE_ORG_ID,"
				+ "SERVICE_ORG_NAME,REGION_CODE,APPLY_NAME,EVALUATE_TYPE) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		try {
			int i;
			i = this.executeUpdate(sql,
	...
	public DataSet insertGuideEvaluation(ParameterSet pSet) {
		...
		if (ds.getTotal() > 0) {
			...
			sql = "update STAR_LEVEL_EVALUATION set STAR_LEVEL=?,EVALUATE_CONTENT=? where service_id=? and evaluate_type='3' ";
			try {
				int i;
				i = this.executeUpdate(sql,
						new Object[] {pSet.getParameter("STAR_LEVEL"), (String) pSet.getParameter("EVALUATE_CONTENT"), pSet.getParameter("ITEM_ID")});
				
				if (0 == i) {
					ds.setState(StateType.FAILT);
					ds.setMessage("更新评价内容失败！");
				}
			} catch (Exception e) {
				ds.setState(StateType.FAILT);
				e.printStackTrace();
			...
			sql = "insert into STAR_LEVEL_EVALUATION(ID,STAR_LEVEL,EVALUATE_CONTENT,"
					...
					+ "SERVICE_ORG_NAME,REGION_CODE,EVALUATE_TYPE) values(?,?,?,?,?,?,?,?,?,?,?)";
			try {
				int i;
				i = this.executeUpdate(sql,
						...
						CommonUtils_api.getInstance().parseDateToTimeStamp(new Date(),
								...
								pSet.get("DEPT_NAME"), pSet.get("REGION_ID"),pSet.get("EVALUATE_TYPE")});
	...
	public DataSet getBjPjList(ParameterSet pSet) {
		...
		StringBuffer str = new StringBuffer();
		str.append("select t.BSR_NAME,t.EVALUATE_CONTENT,t.STAR_LEVEL,t.CREATOR_DATE from star_level_evaluation t ");
