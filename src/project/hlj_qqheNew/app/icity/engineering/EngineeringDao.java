...
public class EngineeringDao extends BaseJdbcDao {
	...
	public DataSet getReport(ParameterSet pSet) {
		...
		try {
			...
			JSONObject obj;
			//url中 % 是默认的转义字符，当字符串中出现 %预付 等字样时会提示错误。
	...
	public DataSet getFlowImageMessage(ParameterSet pSet) {
		...
		String url = HttpUtil.formatUrl(SecurityConfig.getString("ECParallelUrl")+"getFlowImageMessage");
		PostMethod postMethod = new PostMethod(url);
		client.getParams().setContentCharset("UTF-8");	

		postMethod.setParameter("biz_id", (String)pSet.getParameter("biz_id"));

		JSONObject receive = new JSONObject();
		try{
			
			client.executeMethod(postMethod);
			receive =  JSONObject.fromObject(postMethod.getResponseBodyAsString());
			if(receive.get("code").equals("200")){
				//处理返回参数
