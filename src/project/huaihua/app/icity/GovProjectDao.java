...
public class GovProjectDao extends BaseJdbcDao {
	...
	public DataSet getChannelListByName(ParameterSet pSet) {
		String typeName = (String)pSet.getParameter("typeName");
		String sql ="select t.id from PUB_CHANNEL t where t.name = '"+typeName+"'";
		return this.executeDataset(sql, null);
	...
	public DataSet getContentList(ParameterSet pSet) {
		...
		String sql ="select p.NAME,p.ID,p.CID,u.name uname,p.DEPT_NAME,p.DEPT_ID,p.url,to_char(p.ctime,'yyyy-mm-dd') ctime from PUB_CONTENT p,pub_channel u  where p.cid=u.id ";
		if(title.length()>0){
	...
	public DataSet getPowerList(ParameterSet pSet) {
		...
		try {
			...
			System.out.println("看看url:"+url);
			System.out.println("看看参数："+whereValue.toString()+" "+paramValue.toString()+" ");
	...
	public DataSet getQuestion(ParameterSet pSet) {
		String title = (String)pSet.getParameter("SearchName");
		String deptid = (String)pSet.getParameter("deptid");
		String questionId = (String)pSet.getParameter("questionId");
		...
		if(StringUtils.isNotEmpty(questionId)){
			...
			sql+= " and p.cid = '"+questionId+"' ";
			}
		}
		if(StringUtils.isNotEmpty(startTime)){
			if(startTime.length() > 0){
			sql+= "and p.valid_date_start >= to_date('"+startTime+"','yyyy/mm/dd')";
			}
		}
		if(StringUtils.isNotEmpty(endTime)){
			if(endTime.length()>0){
			sql+= "and p.valid_date_end <= to_date('"+endTime+"','yyyy/mm/dd')";
			}
		}
	...
	public DataSet getQuestionById(ParameterSet pSet){
		String questionId = (String)pSet.getParameter("questionId");
		String sql = "select t.name,t.content,to_char(t.ctime,'yyyy-mm-dd') ctime,t.creator from PUB_CONTENT t where t.id = '"+questionId+"'";
		return this.executeDataset(sql,null);
	}
	...
	public DataSet getDwgk(ParameterSet pSet){
		...
		int limit = (Integer)pSet.getParameter("limit");
		String sql = "select t.id,t.name,t.content,to_char(t.ctime,'yyyy-mm-dd') ctime,t.creator from PUB_CONTENT t,pub_channel c where c.id = t.cid and c.name='"+title+"' and c.parent!='root' and t.submit_status='1' ";
	...
	public DataSet getContent_hczw(ParameterSet pSet){
		String id = (String)pSet.getParameter("id");
		String sql = "select t.name,t.content,to_char(t.ctime,'yyyy-mm-dd') ctime,t.creator from PUB_CONTENT t where t.id = '"+id+"'";
		return this.executeDataset(sql,null);
	}
