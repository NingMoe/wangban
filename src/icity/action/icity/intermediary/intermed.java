package action.icity.intermediary;
 ...
 * @author 2015.12.22
 */
public class intermed extends BaseAction {
	...
	public boolean handler(Map<String, Object> data) throws Exception {
		...
		DataSet dSet = UserDao.getInstance().getListExt("" + this.getUserInfo().getUid());
		String name = "";
		String password = "";
		if (dSet.getTotal() > 0) {
			JSONObject uInfo = dSet.getRecord(0);
			name = uInfo.getString("ACCOUNT");
			password = uInfo.getString("PASSWORD");
		}
		String url = SecurityConfig.getString("investment_login")+"?NAME="+strEnc(name, password_DES, "", "")+
				"&PASSWORD="+strEnc(password, password_DES, "", "")+"&SYBMOL="+strEnc("inspur", password_DES, "", "");
		this.sendRedirect(url);
		return false;
	}

	/**
	 * strEnc 加密方法 strDec 解密方法 DES加密/解密
	 */
	...
	public String bt64ToHex(int[] byteData) {
		...
		for (int i = 0; i < 16; i++) {
			...
			for (int j = 0; j < 4; j++) {
				btStr.append(byteData[i * 4 + j]);
			}
			hex.append(bt4ToHex(btStr.toString()));
		}
		return hex.toString();
	}
