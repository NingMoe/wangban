 ...
 *
 */
public class WindowSyncDao extends BaseJdbcDao {

	private WindowSyncDao() {
		this.setDataSourceName("icityDataSource");
	}

	public static WindowSyncDao getInstance() {
		return DaoFactory.getDao(WindowSyncDao.class.getName());
	}
	...
	public DataSet DeleteSyncData(ParameterSet pSet) {
		...
		if (!StringUtils.isNotEmpty(busiCode)) {
			// log_.info("同步删除窗口数据失败！");
			return ds;
		}

		String busiCodeArray[] = busiCode.split(",");

		String sql = "DELETE FROM BASE_WINDOW WHERE CODE=?";
