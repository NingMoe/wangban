...
public class ApplyCloudDao extends BaseJdbcDao{
	...
	private JSONObject changeData(JSONObject data){
	    ...
	    m_data.put("InnerCode", data.containsKey("InnerCode")?(data.getString("InnerCode").contains("null")?"":data.getString("InnerCode")):"");//编办规定事项编码（用于数据交换）（必填）
	    m_data.put("IsPrejudication","11");
	    m_data.put("State", data.getString("state"));
	    m_data.put("applyFrom", "01");//来源 01网上申报
	    return m_data;
	}
	public JSONObject submitData(JSONObject data) throws Exception {	
		System.out.println("------------------data:"+data);
		data = changeData(data);
		System.out.println("转换后-------------------data:"+data);
		...
		try{
			if("sp".equals(state)||"ql".equals(state)){
				//首次提交先生成申办流水号+查询密码
				System.out.println("开始生成流水号----------");
				JSONObject codeInfo = getCodeInfo(data);
				System.out.println("结束生成流水号----------codeInfo:"+codeInfo.toString());
				String status = "00";
				...
				if(!"".equals(o_sblsh)){
					...
					System.out.println("插入网办库办件记录开始----------");
					...
					System.out.println("插入网办库办件记录结束----------");
				}
