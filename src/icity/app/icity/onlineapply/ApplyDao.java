...
public class ApplyDao extends BaseJdbcDao{
	...
	public JSONObject submitData(JSONObject data) throws Exception {
		...
		JSONObject mails_front = new JSONObject();
		JSONObject mails_back = new JSONObject();
		String orderNo = "";
		
		//判断是否需要邮寄（威海、福州、临沂）
		if("weihai".equals(appId) || "fz".equals(appId) || "linyi_wb".equals(appId)){
		...
		//判断是否需要邮寄（漳州）
		if("zhangzhou".equals(appId)){
			emaillist = data.getJSONObject("emaillist");
		}
		//判断是否需要邮寄（舟山）
		if("zs".equals(appId)){
			mails_front = data.getJSONObject("mails_front");
			if(!mails_front.isNullObject()){
				istoEms = "1";
			}
			mails_back = data.getJSONObject("mails");
			if(!mails_back.isNullObject()&&!mails_back.isEmpty()){
				isEms = "1";
				orderNo = mails_back.getString("orderNo");
			}
		}
		...
		try{
			if("sp".equals(state)||"ql".equals(state)){
				...
				cxmm = codeInfo.containsKey("password")?codeInfo.getString("password"):"";//查询密码
				data.put("receiveNum", sblsh);
				data.put("password", cxmm);
				if(!"".equals(o_sblsh)){
					sql="update business_index set sblsh=?,cxmm=?, state='11',SQRZJHM=?,SQRMC=?,LXRXM=?,LXRSJ=?,SBXMMC=?,sbsj=?,signstate=? where sblsh=? ";
					DbHelper.update(sql, new Object[]{sblsh,cxmm, data.get("zjhm"),data.get("sqrxm"),data.get("lxrxm"),data.get("lxrphone"),"关于"+data.get("sqrxm")+data.get("itemName")+"的业务",CommonUtils_api.getInstance().parseDateToTimeStamp(new Date(),CommonUtils_api.YYYY_MM_DD_HH_mm_SS),"1",o_sblsh},conn);
					...
					if("1".equals(istoEms)){
						...
						DbHelper.update(sql, new Object[]{
								...
								"EMS",
								"0",
								txLogisticID,o_sblsh
						}, conn);
						
						//福州、临沂（分寄件和回寄件，所以有两条数据，需要根据类型区分）
						if("fz".equals(appId)||"linyi_wb".equals(appId)){
							updateEms4Fz(ems,isEms,sblsh,data,txLogisticID,txLogisticID2,o_sblsh);
						}
					}
					//漳州
					if("zhangzhou".equals(appId)&&!emaillist.isNullObject()){
						updateEms4Zz(emaillist,data, txLogisticID2, o_sblsh);
					}
					//舟山
					if("zs".equals(appId)){
						updateEms4Zs(mails_front,mails_back,istoEms,isEms,sblsh,data,txLogisticID,orderNo,o_sblsh);
					}
				}else{
					...
					"STATE,URL,LIMIT_NUM,ASSORT,signstate,ly) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,"+
					"?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
						DbHelper.update(sql, new Object[]{
								...
								data.get("assort"),
								"1",//数据交换标志位
								data.get("ly"),//来源，icity.wechat等  xnrtvzsxn（掌上西宁）
							}, conn);
					...
					if("1".equals(istoEms)){
						if("weihai".equals(appId)){
							...
							DbHelper.update(sql, new Object[]{
									sblsh,ems.getString("emsAcceptName"),
									ems.getString("emsAcceptAddress"),
									ems.getString("emsAcceptPhone"),
									ems.getString("emstoName"),
									ems.getString("emstoPhone"),
						...
						}else if("fz".equals(appId)||"linyi_wb".equals(appId)){
							insertEms4Fz(ems,isEms,sblsh,data,txLogisticID,txLogisticID2);
						}
					}
					//漳州
					if("zhangzhou".equals(appId)&&!emaillist.isNullObject()){
						insertEms4Zz(emaillist, sblsh, data, txLogisticID2);
					}
					//舟山
					if("zs".equals(appId)){
						insertEms4Zs(mails_front,mails_back,istoEms,isEms,sblsh,data,txLogisticID,orderNo);
					}
				}
				...
				DbHelper.update(sql, new Object[]{
						CommonUtils_api.getInstance().parseDateToTimeStamp(new Date(),CommonUtils_api.YYYY_MM_DD_HH_MM_SS_SSS),"bqbz".equals(state)?"11":"01",data.get("zjhm"),data.get("sqrxm"),data.get("lxrxm"),data.get("lxrphone"),"关于"+data.get("sqrxm")+data.get("itemName")+"的业务",limit_num,"1","3",o_sblsh }, conn);
	...
	public String saveData(JSONObject data) throws Exception{
		...
		try{
				if(!"".equals(o_sblsh)){
					...
					sql = " update SUB_FOR_EX_APP_INFORMATION set OBJECTTYPE = ?,basecontent = ?,FORMID=?,DATAID=?,EX_DATAID=? where sblsh = ?";
					DbHelper.update(sql, new Object[]{objectType,baseContent,formId,dataId,ex_dataId,o_sblsh}, conn);
				}
				else{
	...
	public JSONObject getCodeInfo(JSONObject data) {
		JSONObject codeInfo;
		String uniteCodeSwitch=SecurityConfig.getString("uniteCodeSwitch","off");
		...
		if("off".equals(uniteCodeSwitch)){
			...
			if("xy".equals(SecurityConfig.getString("AppId"))||"weihai".equals(SecurityConfig.getString("AppId"))||"ytsp".equals(SecurityConfig.getString("AppId"))||"yt".equals(SecurityConfig.getString("AppId"))||"fz".equals(SecurityConfig.getString("AppId"))||"chq".equals(SecurityConfig.getString("AppId"))||"zhangzhou".equals(SecurityConfig.getString("AppId"))){
				...
				url+="/createReceiveNum?itemId="+itemId+"&applyFrom="+handlType;
			}
			...
			String urlCode = HttpUtil.formatUrl(SecurityConfig.getString("CodeServicesUrl")+"/getUniteCode?appCode="
                    +appCode+"&itemId="+data.getString("itemId")+"&regionCode="+data.getString("regionCode")
                    +"&deptCode="+data.getString("orgCode")+"&handlType="+handlType
                    +"&powerType="+data.getString("powerType")
                    +"&token="+token);
	...
	public void sendMessageToUser(String smsContent,String channel,String phone){
	...
	//保存EMS（福州）
	public void insertEms4Fz(JSONObject ems,String isEms,String sblsh,JSONObject data,String txLogisticID,String txLogisticID2) throws Exception{
		Connection conn = DbHelper.getConnection(this.getDataSourceName());
		conn.setAutoCommit(false);
	...
	//更新EMS（福州）订单号两个 待做
	public void updateEms4Fz(JSONObject ems,String isEms,String sblsh,JSONObject data,String txLogisticID,String txLogisticID2,String o_sblsh) throws Exception{
		Connection conn = DbHelper.getConnection(this.getDataSourceName());
		conn.setAutoCommit(false);
		try{
	...
	//保存EMS（漳州）
	public void insertEms4Zz(JSONObject ems,String sblsh,JSONObject data,String txLogisticID) throws Exception{
		Connection conn = DbHelper.getConnection(this.getDataSourceName());
		conn.setAutoCommit(false);
		try{
			String is_rate = ems.getString("is_rate");
			String is_catalog = ems.getString("is_catalog");
			String is_rate_state = "0";
			String is_catalog_state = "0";
			//调审批 
			String url =SecurityConfig.getString("approval_url")+"updateZzk";
			if("1".equals(is_rate)){//上门取件
				JSONObject parms = new JSONObject();
				parms.put("id", "");
				parms.put("bsnum", "");
				parms.put("receiveNumber", sblsh);
				parms.put("eType", "R");//R-上门取件;C-送达上门
				parms.put("expressNumber", "");
				parms.put("userName", ems.getString("sender"));
				parms.put("phone", ems.getString("sender_phone"));
				parms.put("address", ems.getString("sender_address"));
				parms.put("status", "1");
				parms.put("context", ems.getString("content"));
				parms.put("remark", ems.getString("remark"));
			...
			if("1".equals(is_catalog)){//办结送件
				JSONObject parms = new JSONObject();
				parms.put("id", "");
				parms.put("bsnum", "");
				parms.put("receiveNumber", sblsh);
				parms.put("eType", "C");//R-上门取件;C-送达上门
				parms.put("expressNumber", "");
				parms.put("userName", ems.getString("reciever"));
				parms.put("phone", ems.getString("reciever_phone"));
				parms.put("address", ems.getString("reciever_address"));
				parms.put("status", "1");//1发起2已录入3取消
				parms.put("context", ems.getString("content2"));
				parms.put("remark", ems.getString("remark2"));
				Map<String, String> map = new HashMap<String, String>();
				map.put("postdata", parms.toString());
				log.error("办结送件政证快："+url);
				log.error("办结送件政证快：参数："+map);
				JSONObject ret = JSONObject.fromObject(RestUtil.postData(url, map));
				log.error("办结送件政证快：返回结果："+ret);
				if("200".equals(ret.get("state"))){
					is_catalog_state ="1";//成功
				}
			}
			//网办保存数据
			if("1".equals(is_rate)){//上门取件
				String sql = "insert into business_express (SBLSH,SENDER,SENDER_PHONE,SENDER_ADDRESS,OPERATOR,OPERRATE_DATE,COM,ISTAKEN,ORDER_NUMBER,SEND_TYPE,CONTENT,REMARK) values(?,?,?,?,?,?,?,?,?,?,?,?)";
				DbHelper.update(sql, new Object[]{
						sblsh,
						ems.getString("sender"),
						ems.getString("sender_phone"),
						ems.getString("sender_address"),
						data.get("ucid"),
						CommonUtils_api.getInstance().parseDateToTimeStamp(new Date(),CommonUtils_api.YYYY_MM_DD_HH_MM_SS_SSS),
						"EMS",
						is_rate_state,
						txLogisticID,
						"0",
						ems.getString("content"),
						ems.getString("remark")
				}, conn);
			}
			if("1".equals(is_catalog)){//办结送件
				String sql2 = "insert into business_express (SBLSH,RECIEVER,RECIEVER_ADDRESS,RECIEVER_PHONE,OPERATOR,OPERRATE_DATE,COM,ISTAKEN,ORDER_NUMBER,SEND_TYPE,CONTENT,REMARK) values(?,?,?,?,?,?,?,?,?,?,?,?)";
				DbHelper.update(sql2, new Object[]{
						sblsh,
						ems.getString("reciever"),
						ems.getString("reciever_address"),
						ems.getString("reciever_phone"),
						data.get("ucid"),
						CommonUtils_api.getInstance().parseDateToTimeStamp(new Date(),CommonUtils_api.YYYY_MM_DD_HH_MM_SS_SSS),
						"EMS",
						is_catalog_state,
						txLogisticID,
						"1",
						ems.getString("content2"),
						ems.getString("remark2")
				}, conn);
			}
			conn.commit();
	    }catch(Exception e){
	...
	//更新EMS（漳州）
	public void updateEms4Zz(JSONObject ems,JSONObject data,String txLogisticID,String o_sblsh) throws Exception{
		Connection conn = DbHelper.getConnection(this.getDataSourceName());
		conn.setAutoCommit(false);
		String is_rate = ems.getString("is_rate");
		String is_catalog = ems.getString("is_catalog");
		...
		try{
			//调审批 
			String geturl =SecurityConfig.getString("approval_url")+"getZzk?regionCode="+SecurityConfig.getString("WebRegion")+"&receiveNumber="+o_sblsh;
			JSONObject getret = JSONObject.fromObject(RestUtil.getData(geturl));
			JSONArray list = (JSONArray) getret.get("data");
			//上门取件
			String r_ID ="";//真正快业务主键
			String r_BSNUM ="";//审批业务主键
			String r_EXPRESS_NUMBER ="";//快递单号
			//办结送件
			String c_ID ="";//真正快业务主键
			String c_BSNUM ="";//审批业务主键
			String c_EXPRESS_NUMBER ="";//快递单号
			
			if(list.size()>0){
				for(int i = 0;i<list.size();i++){
					JSONObject obj = (JSONObject) list.get(i);
					//快递类型：R-上门取件;C-送达上门
					if(obj.get("RECEIVE_NUMBER").equals(o_sblsh)&&obj.get("E_TYPE").equals("R")){
						r_ID = (String) obj.get("RECEIVE_NUMBER");
						r_BSNUM =(String) obj.get("RECEIVE_NUMBER");
						r_EXPRESS_NUMBER =(String) obj.get("RECEIVE_NUMBER");
					}
					if(obj.get("RECEIVE_NUMBER").equals(o_sblsh)&&obj.get("E_TYPE").equals("C")){
						c_ID = (String) obj.get("RECEIVE_NUMBER");
						c_BSNUM =(String) obj.get("RECEIVE_NUMBER");
						c_EXPRESS_NUMBER =(String) obj.get("RECEIVE_NUMBER");
					}
				}
			}
			...
			if("1".equals(is_rate)){//上门取件
				JSONObject parms = new JSONObject();
				parms.put("id", r_ID);
				parms.put("bsnum", r_BSNUM);
				parms.put("receiveNumber", o_sblsh);
				parms.put("eType", "R");//R-上门取件;C-送达上门
				parms.put("expressNumber", r_EXPRESS_NUMBER);
				parms.put("userName", ems.getString("sender"));
				parms.put("phone", ems.getString("sender_phone"));
				parms.put("address", ems.getString("sender_address"));
				parms.put("status", "1");
				parms.put("context", ems.getString("content"));
				parms.put("remark", ems.getString("remark"));
				JSONObject ret = JSONObject.fromObject(RestUtil.postData(url, parms));
				if("200".equals(ret.get("state"))){
					is_rate_state ="1";//成功
				}
			}
			if("1".equals(is_catalog)){//办结送件
				JSONObject parms = new JSONObject();
				parms.put("id", c_ID);
				parms.put("bsnum", c_BSNUM);
				parms.put("receiveNumber", o_sblsh);
				parms.put("eType", "C");//R-上门取件;C-送达上门
				parms.put("expressNumber", c_EXPRESS_NUMBER);
				parms.put("userName", ems.getString("reciever"));
				parms.put("phone", ems.getString("reciever_phone"));
				parms.put("address", ems.getString("reciever_address"));
				parms.put("status", "1");//1发起2已录入3取消
			...
			if("1".equals(is_rate)){//上门取件
				String sql = "update business_express set SENDER=?,SENDER_PHONE=?,SENDER_ADDRESS=?,OPERATOR=?,OPERRATE_DATE=?,COM=?,ISTAKEN=?,ORDER_NUMBER=?,CONTENT=?,REMARK=? where SBLSH=? and send_type=0 ";
				DbHelper.update(sql, new Object[]{
						ems.getString("sender"),
						ems.getString("sender_phone"),
						ems.getString("sender_address"),
			...
			if("1".equals(is_catalog)){//办结送件
				String sql2 = "update business_express set RECIEVER=?,RECIEVER_ADDRESS=?,RECIEVER_PHONE=?,OPERATOR=?,OPERRATE_DATE=?,COM=?,ISTAKEN=?,ORDER_NUMBER=?,CONTENT=?,REMARK=? where SBLSH=? and send_type=1 ";
				DbHelper.update(sql2, new Object[]{
						ems.getString("reciever"),
						ems.getString("reciever_address"),
						ems.getString("reciever_phone"),
					...
					String action;
						for (int i = 0; i < ds.getTotal(); i++) {
							try {
								...
								if ("99".equals(obj.get("ACTION")) || "98".equals(obj.get("ACTION"))) {
									action="10";
								}
								...
								try {
									...
									if (json.getString("state").equals("1")) {
										...
										h = DbHelper.update(sql, new Object[] { obj.get("RECEIVENUM") }, conn);
										if(h > 0){
											conn.commit();
										}else{
											conn.rollback();
	...
	//保存EMS（舟山）
	public void insertEms4Zs(JSONObject mails_front,JSONObject mails_back,String istoEms,String isEms,String sblsh,JSONObject data,String txLogisticID,String orderNo) throws Exception{
		Connection conn = DbHelper.getConnection(this.getDataSourceName());
		conn.setAutoCommit(false);
		try{
			String sql = "insert into business_express (SBLSH,RECIEVER,RECIEVER_ADDRESS,RECIEVER_PHONE,SENDER,SENDER_PHONE,OPERATOR,OPERRATE_DATE,COM,ISTAKEN,ORDER_NUMBER,SEND_TYPE) values(?,?,?,?,?,?,?,?,?,?,?,?)";
...
//			}, conn);
			...
			if("1".equals(isEms)){
				...
				JSONObject receiver = mails_back.getJSONObject("receiver");
				String address = receiver.getString("provinceName")+receiver.getString("cityName")+receiver.getString("regionName")+receiver.getString("address");
				sql = "insert into business_express (SBLSH,RECIEVER,RECIEVER_ADDRESS,RECIEVER_PHONE,SENDER,SENDER_PHONE,OPERATOR,OPERRATE_DATE,COM,ISTAKEN,ORDER_NUMBER,SEND_TYPE) values(?,?,?,?,?,?,?,?,?,?,?,?)";
				DbHelper.update(sql, new Object[]{
						sblsh,
						receiver.getString("name"),
						address,
						receiver.getString("phone"),
						data.get("windowName"),
						data.get("windowPhone"),
						data.get("ucid"),
						CommonUtils_api.getInstance().parseDateToTimeStamp(new Date(),CommonUtils_api.YYYY_MM_DD_HH_MM_SS_SSS),
						"EMS",
						"0",
	...
	//更新EMS（舟山）
	public void updateEms4Zs(JSONObject mails_front,JSONObject mails_back,String istoEms,String isEms,String sblsh,JSONObject data,String txLogisticID,String orderNo,String o_sblsh) throws Exception{
		Connection conn = DbHelper.getConnection(this.getDataSourceName());
		conn.setAutoCommit(false);
		try{
			String sql = "update business_express set RECIEVER=?,RECIEVER_ADDRESS=?,RECIEVER_PHONE=?,SENDER=?,SENDER_PHONE=?,OPERATOR=?,OPERRATE_DATE=?,COM=?,ISTAKEN=?,ORDER_NUMBER=? where SBLSH=? and send_type=0 ";
