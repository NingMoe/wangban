...
public class ApplyCmd_bak extends BaseQueryCommand {
	private static Log _log = LogFactory.getLog(ApplyCmd_bak.class);
	...
	public DataSet submitSP(ParameterSet pSet) {
		...
		try{
				...
				if(receiveNum.get("state").equals("200")){
					ApplyDao_bak.getInstance().updateBusinessAttach(receiveNum,data);
					ApplyDao_bak.getInstance().submitData(o_sblsh,receiveNum,data);
	...
	public DataSet saveDraft(ParameterSet pSet){
		...
		JSONObject data =((JSONObject)pSet.get("data"));
		try{
					String o_sblsh=ApplyDao_bak.getInstance().saveData(data);
