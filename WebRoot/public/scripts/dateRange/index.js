var $, Calendar, DAYS, DateRangePicker, MONTHS, TEMPLATE;
  ...
  DateRangePicker = (function() {
    function DateRangePicker($drp) {
      this.$drp = $drp;
      this.$dateRangePicker = $(TEMPLATE);
      this.$drp.attr('tabindex', '-1').before(this.$dateRangePicker);
      this.isHidden = true;
      this.initBindings();
      this.setRange(0);
    }
    ...
    DateRangePicker.prototype.initBindings = function() {
      var self;
      self = this;
      this.$drp.on('focus mousedown', function(e) {
        var $drp;
        $drp = this;
        setTimeout(function() {
          return $drp.blur();
      ...
      $('body').click(function(evt) {
        if (evt.target === self.$drp[0] && self.isHidden) {
          return self.show();
...
function drp() {
	if(typeof(myDateRangePicker) == "undefined"){
		myDateRangePicker = $('.custom-date').dateRangePicker();
	}
}
