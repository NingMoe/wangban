...
function cidInfo(sId) {
    ...
    if (!(/(^\d{15}$)|(^\d{17}([0-9]|X)$)/.test(sId))) {
    	$("#carderr").html("<font color='red'>输入的身份证号长度不对，或者号码不符合规定！\n15位号码应全为数字，18位号码末位可以为数字或X</font>");
        return false;
    }
    sId = sId.replace(/x$/i, "a");
    if (aCity[parseInt(sId.substr(0, 2))] == null) { $("#carderr").html("<font color='red'>系统提示,非法地区</font>"); return false; }
    sBirthday = sId.substr(6, 4) + "-" + Number(sId.substr(10, 2)) + "-" + Number(sId.substr(12, 2));
    var d = new Date(sBirthday.replace(/-/g, "/"));
    if (sBirthday != (d.getFullYear() + "-" + (d.getMonth() + 1) + "-" + d.getDate())) {$("#carderr").html("<font color='red'>系统提示:非法生日</font>"); return false; }
    if (iCount == 18) {
        for (var i = 17; i >= 0; i--) iSum += (Math.pow(2, i) % 11) * parseInt(sId.charAt(17 - i), 11);
        if (iSum % 11 != 1) { $("#carderr").html("<font color='red'>证件输入不合法</font>"); return false; }
    }
    $("#carderr").html("");
    return true;
}
...
//jquery验证手机号码 
function checkSubmitMobil(o) { 
if ($(o).val() == "") { 
$("#moileMsg").html("<font color='red'>手机号码不能为空！</font>"); 
return false; 
}
else if (!$(o).val().match(/^[0-9]{11}$/)) { 
$("#moileMsg").html("<font color='red'>手机号码格式不正确！请输入11位手机号！</font>"); 
return false; 
}
else{
$("#moileMsg").html(""); 
