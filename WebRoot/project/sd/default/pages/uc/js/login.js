$(document).ready(function() {
	$.ajax({
		...
		error : function() {
			$("#verify_code_div_gm").hide();
			$("#verify_code_div_fr").hide();
		}
	});
	...
	if(document.getElementById('password')){
		document.getElementById('password').onkeydown = function(event) {
			...
			if (e && e.keyCode == 13) {
				if (($("#verify_code_div_gm").css("display") == "none") || ($("#verify_code_div_fr").css("display") == "none")) {
					okSubmit();
				} else if (($("#verify_code_div_gm").css("display") == "block") ||($("#verify_code_div_fr").css("display") == "block")) {
...
function changeVerify() {
	...
	$("#verifyImg_gm").attr("src", url);
	$("#verifyImg_fr").attr("src", url);
}
...
function showVerifyCode(obj) {
	if (obj && obj.data && obj.data.login_fail_time) {
		...
		if (loginFailTime > 3) {
			$("#verify_code_div_gm").show();
			$("#verify_code_div_fr").show();
			changeVerify();
...
function okSubmit() {
	var type = $(".form-tab li.on").attr("tag");// 登录类型
		...
		// 表单验证
		if ((!$("#verify_code_div_gm").is(":hidden"))|| (!$("#verify_code_div_fr").is(":hidden"))) {
