...
$(function(){	
	...
	*/
	...
	setSelected(document.getElementById('projectType'),"备案类项目");
	if(neworold=="新建"){
...
function checkit(){
	...
	var constructPer=$("#constructPer").find("option:selected").text().trim();
	if(constructPer=="") {
		$("#constructPer").addClass("bordered");}
	else{
		$("#constructPer").removeClass("bordered");
	}
    //modify by cys,注释以下内容
    ...
    //modify by cys,for:提交时，判断输入框不为空
	...
	var projectCapitalMoney = $("#projectCapitalMoney").val().trim();//项目资本金
	...
	var mergerPlan = $("#mergerPlan").val();// 并购安排
	...
	if (types == "外商投资") {
	    ...
	    if (totalMoneyDollar == "" || !validateY(totalMoneyDollar) || totalMoneyDollarRate == "" || !validateY(totalMoneyDollarRate) ||
			...
			|| otherInvestmentApplyInfo == "" || builtArea == ""||!validateY(builtArea)||!validateY(landArea)||landArea=="" || importDeviceNumberMoney == "" || theIndustryName == "" || contactTel == "" || contactFax == "") {
	        alert("请将必填项填写之后再进行提交。");
	        return false;
	    }
...
function submit(){
	...
	obj.divisionCode = formatLen(obj.divisionCode,0,12);
		if(obj.divisionCode.length!=12){	
			obj.divisionCode=obj.divisionCode+"000000";
      }
...
function btnoklerepIfo(tab) {
    ...
    $("#cover").hide();
   /* addlerepInfoJson = {
        "id": rowCount, "enterpriseName": enterpriseName, "lerepCerttype": lerepCerttype,
...
function btnokcontrInfo(tab) {
    ...
    var regCountry = $("#regCountry").val().trim();
    if (regCountryName == "") {
    ...
    var contributionLimit = $("#contributionLimit").val();
   ...
   /* addcontrInfoJson = {
        "id": rowCountcontrInfo, "businessType": businessType, "investmentName": investmentName, "regCountry": regCountry,
