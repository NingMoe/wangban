var icityUrl = window.location.protocol+"//"+window.location.host+"{{PageContext.ContextPath}}";
//网办内网访问外网资源时，IE10以下版本 LEx.webPath 获取的是内网的url.报404，使用icityUrl代替
function init() {
		onQuery(); // 执行默认的查询方法
}
...
function editSite(name) {
	...
	var url = icityUrl+'ipmi?m=pmi/config/editsite.html&name=' + name;
	LEx.dialog({
		title : "修改站点",
		url : url,
		lock : true,
...
function addsite(pid,level) {
	var url = icityUrl+'ipmi?m=pmi/config/addsite.html&pid=' + pid;
	LEx.dialog({
		title : "添加站点",
		url : url,
		lock : true,
