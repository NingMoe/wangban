	...
	if(zjlx==10){
		if(!cidInfo($(o).val())){
			...
			$(o).removeClass("input_validation-invalid").addClass("input_validation-valid");
				$(o).val(v.substr(0,5)+"***********"+v.substr(16,18));
			return "success";
		}
	}
...
function getAttachName(name,uid){
	var cmd = new LEx.Command("app.icity.project.WdwpCmd");
	cmd.setParameter("name", name);
	cmd.setParameter("UID", uid+"");
	var ret =  cmd.execute("getAttachName");
	return ret;	
}
/**
* 获取焦点的时候恢复数字
...
function validate(o) {
	...
	if(!reg.test(val)){
		...
		if($(o).attr("name")=="linkPhone"){			
		    ...
		    if(!regNumber.test($("#linkPhone").val())){
		    	$(o).removeClass("input_validation-valid").addClass("input_validation-invalid");
		    	return false;
		    }
		 }
		if($(o).attr("name")=="contactPhone"){			
			 ...
			 $(o).val(val.substr(0,3)+"****"+val.substr(7,7));
			 var regNumber = new RegExp("^[0-9]*$");
			 if(!regNumber.test($("#contactPhone").val())){
				$(o).removeClass("input_validation-valid").addClass("input_validation-invalid");
			    return false;
			 }
		 }
...
function loadInfo(type) {
	...
	if(type=="1") {
		if("{{ConfigInfo.AppId}}"!="zs"){
			...
			if(u_type == "11") {
				$("#name").val(userInfo.userName);//姓名
				$("[name=linkPhone]").val(userInfo.mobile);//联系电话
			}
			/*$("#orgName").val(userInfo.userName);//组织机构名称
			$("#orgCode").val(userInfo.userId);//组织机构代码
*/		}
	}
	//企业
	else if(type=="3") {
		...
		add : function(e, data) {
			...
			$.each(data.files, function(i, file) {
				...
				if(file.size>fileSize){
					...
					var ret = getAttachName(file.name,LEx.userInfo.uid);
					if(ret.state=="1"){
						if(ret.total>0){
							if(confirm("文件已存在，是否继续上传？")){							
								return check=true;
							}else{
								return check=false;
							}
...
function uploadSuccess(e, serverData) {
	...
	for(var i=0;i<serverData.result.length;i++){
		...
		var ret = getAttachName(data.name,LEx.userInfo.uid);
		if(ret.state=="1"){
			if(ret.total>0){
				data.yname = ret.data;
			}
		}
		$("#list_"+material_code).append("<div id=\"file_"+data.scc.docid+"\">"+formatLenStr(data.yname,0,5)+"<a class='btn btn-success btn-sm' href=\"javascript:void(0)\" onclick=\"onDownload(this);\">下载</a> | <a class='btn btn-danger btn-sm' href=\"javascript:void(0)\" onclick=\"onDel(this);\">删除</a>"+"<br/><br/></div>");
