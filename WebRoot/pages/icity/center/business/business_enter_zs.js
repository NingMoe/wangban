...
function onQuery(){
	...
	if(LEx.isNotNull(sbbh)){
		cmd.setParameter("RECEIVE_ID@like",sbbh);
   }
...
BusinessDetail=function(id, name, applyTime, state){
	/*
	 * type 0:业务列表      1：企业设立        2：工程建设 
	 */
	var n_url = LEx.webPath+"admin?m=icity/center/business/businessengindetail.html&id="+id+"&name="+name+"&APPLY_TIME="+applyTime+"&BIZ_STATE="+state+"&type=1";
		LEx.dialog({
			id:'',
			title:"业务详情",
			url:n_url,
...
LhtkDetail=function(id,receiveid){
	var n_url = LEx.webPath+"admin?m=icity/center/business/lhtk_detail.html&id="+id+"&receiveid="+receiveid;
		LEx.dialog({
			id:'',
			title:"联合踏勘事项",
			url:n_url,
...
function fomatterDate(val){
	if(LEx.isNotNull(val)){		
		return val;//LEx.util.Format.formatDate(val,"yyyy-MM-dd hh:mm:ss");
...
function parseLength(val) {
	if (LEx.isNotNull(val)&&val.length>15) {
