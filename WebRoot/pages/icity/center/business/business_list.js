...
function onQuery(){
	...
	if(LEx.isNotNull(sblsh)){
		cmd.setParameter("t.SBLSH@like",sblsh);
   }
   ...
   cmd.setParameter("sort"," SBSJ DESC");
	...
	if(!ret.error){			
		...
		$(".paycontentDiv").hover(function() {
		    $(this).find("em").animate({position:'absolute',opacity: "show"}, "slow");
		 }, function() {
...
function showDetail(docid,filename){
	
	var url = "{{PageContext.ContextPath}}bsp/uploadify?action=downloadagent4wp&path="+docid+"&name="+filename+"&flag=1";
	//var url = "{{ConfigInfo.NetDiskDownloadAddress}}"+docid+"&type=thumbnail";
	window.open(url);
}
