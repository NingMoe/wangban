		...
		$.ajax({
			...
			success : function(data) {
				if(data == '-1'){
					...
					$("#verify_btn").addClass('opt_btn opt_btn_gray');
					phoneSettime();
...
//	});
	...
	var submit = function(){
		...
		$.ajax({
			...
			success : function(data) {
				if(data == '0') {	
					$("#verify-dialog-phone").find("div[name='verify-tips']")[0].firstChild.nodeValue = "短信校验码错误，请重新输入或者重新获取。";
					$("#verify-dialog-phone").dialog('open');
				}
				else if(data == '1'){
