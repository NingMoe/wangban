	...
	var submit = function(){
		...
		$.ajax({
			...
			success : function(data) {
				if(data=='-1') {	
					$("#verify-dialog-que").find("div[name='verify-tips']")[0].firstChild.nodeValue = "验证码不正确。";
					$("#verify-dialog-que").dialog('open');
				}else if(data=='-2'){
					$("#verify-dialog-que").find("div[name='verify-tips']")[0].firstChild.nodeValue = "密保问题答案错误，请重新填写。";
					$("#verify-dialog-que").dialog('open');
				}else if(data="1"){
